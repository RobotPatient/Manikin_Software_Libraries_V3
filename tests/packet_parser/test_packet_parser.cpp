...
TEST_CASE("packet_parser_encapsulate produces correct packet", "[packet_parser][REQ-F1]") {
    packet_parser_cmd_t cmd = {
        .len  = static_cast<uint16_t>(test_cbor_len),
        .data = test_cbor_data
    };
...
TEST_CASE("packet_parser_parse detects CRC failure", "[packet_parser][REQ-F1]") {
    uint8_t corrupted_packet[64] = {0x01, 0x04, 0x3A, 0xA1, 0x01, 0x18, 0x2B, 0x00, 0x00, 0x17}; // corrupted CRC
...
TEST_CASE("packet_parser_encapsulate fails on nulls", "[packet_parser][REQ-F1]") {
    ...
    packet_parser_cmd_t null_data_cmd = {
        .len = 4,
        .data = nullptr
    };
    REQUIRE(packet_parser_encapsulate(&null_data_cmd, out_buf, sizeof(out_buf)) == MANIKIN_STATUS_ERR_NULL_PARAM);

    packet_parser_cmd_t valid_cmd = {
        .len = 4,
        .data = test_cbor_data
    };
    REQUIRE(packet_parser_encapsulate(&valid_cmd, nullptr, sizeof(out_buf)) == MANIKIN_STATUS_ERR_NULL_PARAM);
}
