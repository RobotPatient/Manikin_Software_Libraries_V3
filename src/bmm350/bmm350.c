...
bmm350_parse_raw_data (const uint8_t *raw_data, bmm350_sample_data_t *data)
{
    ...
    data->temperature_ut = CONSTRUCT_SIGNED_24BIT(raw_data[11], raw_data[10], raw_data[9]);
    data->sensor_time_ut = CONSTRUCT_SIGNED_24BIT(raw_data[14], raw_data[13], raw_data[12]);
    return MANIKIN_STATUS_OK;
}